...
public class DroneManager implements DroneManagerInterface {
	...
	/**
	 * keeping available drones; key - drone id; value - drone and it's
	 * quantity.
	 * 
	 */
	...
	/**
	 * Keeping unavailable drones. key - drone id. value drone and its quantity.
	 */
	...
	public void addAvailableDrone(Drone drone, int quantity) {
		...
		if (getAvailableDrones().containsKey(drone.getId())) {
			...
			int newQuantity = getAvailableDrones().get(drone.getId()).getQuantity() + quantity;
	...
	public void addUnavailableDrone(Drone drone, int quantity) {
		...
		if (getUnavailableDrones().containsKey(drone.getId())) {
			...
			int newQuantity = getUnavailableDrones().get(drone.getId()).getQuantity() + quantity;
	...
	public Map<Integer, DroneWrapper> getAssignedDrones(double weight) {
		...
		for (Map.Entry<Integer, DroneWrapper> entry : availableDrones.entrySet()) {
			...
			if (dronesNumber <= dronesQuantity) {
				...
				assignedDrones.put(thisDrone.getDrone().getId(), new DroneWrapper(thisDrone.getDrone(), dronesNumber));
			...
			/**
				 * while (dronesQuantity > 0) { if (capacity >= weight) { return
				 * assignedDrones; } capacity +=
				 * thisDrone.getDrone().getCapacity();
				 * assignedDrones.add(thisDrone.getDrone()); dronesQuantity--; }
				 **/
		}
